...
SETTINGS_GROUPS: tuple[SettingsGroup, ...] = (
    ...
    SettingsGroup(
        ...
        (
            ...
            # Пусто = та же модель, что у репост-бота. Своя нужна затем, что
            # классификация спама проще рерайта и дешёвая модель с ней
            # справляется. Адрес и ключ при этом ОБЩИЕ, из админки
            # репост-бота (см. guardian/config.py::_shared_ai_override).
            SettingField(
                "openai_model", "Модель (пусто — как у репост-бота)", "str"
            ),
        ),
        "keywords — бесплатно, только стоп-слова. ai — каждое сообщение "
        "через LLM (дороже всего). hybrid (рекомендуется) — эвристики "
        "отбирают ~20% подозрительных, только они идут в AI. Адрес и ключ "
        "провайдера общие с репост-ботом — задаются в его /settings, "
        "здесь можно только выбрать другую модель.",
    ),
    SettingsGroup(
        "captcha",
        "Капча (G01)",
    ...
    SettingsGroup(
        ...
        "Автоответчик (F45)",
        (
            SettingField("autoreply_enabled", "Включить автоответчик", "bool"),
            SettingField("autoreply_rules", "Правила (JSON)", "text"),
            SettingField("autoreply_cooldown_seconds", "Пауза на правило, сек", "int"),
        ),
        'Снимает рутину: «как купить», «где правила» спрашивают каждый день. '
        'Формат правил — JSON-массив: '
        '[{"triggers": ["правила", "rules"], "reply": "Правила в закрепе"}]. '
        'Срабатывает по СЛОВУ целиком, а не по подстроке («стрим» не стрельнет '
        'на «экстримальный»). Пауза не даёт боту отвечать десять раз подряд на '
        'один и тот же вопрос от разных людей. На сообщения других ботов не '
        'отвечает — иначе два бота устроят бесконечный обмен.',
    ),
    SettingsGroup(
        "force_subscribe",
        "Обязательная подписка на канал (F61)",
        (
            SettingField("force_subscribe_enabled", "Включена", "bool"),
            SettingField(
                "force_subscribe_channel", "@канал или его chat_id", "str",
            ),
        ),
        "Участник не может писать в группе, пока не подписан на канал — "
        "прямая воронка «участник группы → подписчик канала». БОТ ОБЯЗАН "
        "БЫТЬ АДМИНИСТРАТОРОМ В КАНАЛЕ, иначе проверка не отработает; в "
        "этом случае сообщения ПРОПУСКАЮТСЯ, а не блокируются — перекрыть "
        "чат из-за сетевой ошибки хуже, чем пропустить неподписанного. "
        "Администраторы группы освобождены: владелец, забывший подписаться "
        "на собственный канал, не должен упереться в свой же запрет. "
        "Человеку показывается ссылка и объяснение — молча удалённое "
        "сообщение выглядит как поломка, а не как правило.",
    ),
    SettingsGroup(
        "spam_learning",
        "Обучение антиспама (F57)",
        (
            SettingField("spam_learning_enabled", "Включено", "bool"),
            SettingField(
                "spam_learning_examples_per_label", "Примеров каждой метки", "int",
            ),
        ),
        "Спорные вердикты AI-фильтра уходят в лог-канал с кнопками «спам / "
        "не спам», а размеченные примеры подмешиваются в промпт. Ничего не "
        "удаляет и не банит — только наблюдает. Примеров берётся поровну "
        "каждой метки: перекос в сторону спама научил бы модель называть "
        "спамом всё подряд.",
    ),
    SettingsGroup(
        "premium_trust",
        ...
        (
            SettingField("premium_trust_enabled", "Включён", "bool"),
            SettingField("premium_trust_bonus", "Насколько смягчать", "int"),
        ),
        "Premium-аккаунту капча даётся мягче: у скам-ботов платной подписки "
        "обычно нет. ВЫКЛЮЧЕНО по умолчанию — это ослабление защиты, а "
        "Premium покупается, поэтому сигнал идёт в общий скоринг, а не даёт "
        "пропуск.",
    ),
    SettingsGroup(
        "hygiene",
