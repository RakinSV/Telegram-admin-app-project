...
async def on_message(message: Message, bot: Bot) -> None:
    ...
    # F42: второе из двух условий реферала — «написал». Отмечается ИМЕННО
    # ЗДЕСЬ, а не в начале обработчика: выше стоят все детерминированные
    # фильтры (форварды, флуд, дубли, ссылки, стоп-слова), и сообщение,
    # которое их не прошло, живым участием не считается — иначе условие
    # обходилось бы одним спам-сообщением.
    #
    # Отметка стоит ДО проверки `not text`, чтобы фотография или голосовое
    # тоже считались участием: человек, приславший фото, не менее живой, чем
    # написавший строку.
    #
    # ИИ-проверка идёт ниже и может удалить сообщение задним числом — отметку
    # это не снимает. Мы фиксируем «прошёл очевидные фильтры», а не «признан
    # хорошим»: вероятностная оценка не должна распоряжаться начислениями.
    try:
        from tg_repost import referrals_repo

        referrals_repo.mark_first_message(user_id)
    except Exception as exc:  # noqa: BLE001
        logger.warning("F42: не удалось отметить первое сообщение %s: %s", user_id, exc)
