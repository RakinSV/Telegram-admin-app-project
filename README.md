...
хранилище настроек, read-only дашборд) + **Фаза 5.2** (жизненный цикл
компонентов из админки — рестарт listener/бота без перезапуска процесса,
визард Telethon-логина в браузере). Подфазы 5.3–5.4 (CRUD-страницы, живые
логи) — впереди, план в `C:\Users\Admin767\.claude\plans\spicy-noodling-eich.md`.
...
(опционально сразу) минимум секретов. Telethon-аккаунт подключается прямо в
браузере — кнопка «Войти через Telegram» (телефон → код → опционально пароль
2FA). Всё, что оставишь пустым на `/setup`, можно заполнить позже на `/secrets`.
...
Применяются сразу (live); поля с пометкой `resync` дополнительно
автоматически синхронизируют состав/расписание джобов планировщика при
сохранении группы (Фаза 5.2), если компоненты уже запущены.

**Компоненты** (`/components`, Фаза 5.2) — статус listener/бота/планировщика
и кнопки живого рестарта каждого без перезапуска процесса: после смены
`TG_SESSION_STRING`/`TG_BOT_TOKEN` на `/secrets` нажми «Перезапустить» —
джобы планировщика, держащие ссылку на старый клиент/бота, автоматически
переключаются на новые экземпляры. Отсюда же — «Сменить Telegram-аккаунт»
(тот же визард входа, что и на `/setup`).
...
python -m tg_repost.tools.backup --keep 30
```
Собирает `.env` + обе SQLite БД (`DATABASE_URL`/`GUARDIAN_DATABASE_URL`) +
`logs/` в один zip в `backups/` (не коммитится, см. `.gitignore`), удаляет
бэкапы сверх `--keep`. `.env` бэкапится всегда вместе с БД — без него
зашифрованные через `WEBUI_MASTER_KEY` секреты в БД невосстановимы.
Для автоматизации — cron/Task Scheduler, например ежедневно в 03:00:
```bash
0 3 * * * cd /path/to/repo && python -m tg_repost.tools.backup
```
⚠️ Архив не шифруется (права файла ограничены до `0600` на POSIX, но это не
замена шифрованию) — не синхронизировать `backups/` в облако/на другую
машину без отдельного шифрования (`age`/GPG).
...
tg_repost/
  ...
  webui/                # веб-админка (F23, Фаза 5.1-5.2)
    app.py               # FastAPI app: /setup /login /settings /secrets / /components
    auth.py              # пароль (Argon2id), сессии, require_login
    settings_store.py    # запись настроек/секретов, SETTINGS_GROUPS
    supervisor.py         # жизненный цикл Telethon/бота/планировщика (Фаза 5.2)
    telethon_login.py     # веб-обвязка пошагового Telethon-визарда (Фаза 5.2)
  ...
  tools/
    gen_session.py     # генерация Telethon session string (CLI + переиспользуемые
                        # шаги для веб-визарда /setup/telethon, Фаза 5.2)
