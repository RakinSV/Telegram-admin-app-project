...
_POLICY: dict[str, str] = {
    ...
    "/crypto": ROLE_OWNER,
    # Боты-конструкторы: здесь ЛЕЖАТ ТОКЕНЫ ботов, то есть полный доступ к
    # ним у Telegram. Тот же уровень, что секреты, — и записано явно, чтобы это
    # было решением, а не следствием умолчания.
    "/bots": ROLE_OWNER,
    # Сценарии живут отдельным адресом (холст открывается по `/flows/{id}`), и
    # правка сценария — это правка того, что бот говорит живым людям.
    "/flows": ROLE_OWNER,
