...
def build_crud_router() -> APIRouter:
    ...
    @router.post("/export/backup/download")
    async def export_backup_download(request: Request) -> Response:
        """Снять полный бэкап и отдать файлом.

        МЕТОД POST, А НЕ GET, хотя это «скачивание». Обработчик не читает
        готовый файл, а СОЗДАЁТ его: собирает .env, базы и логи в архив на
        диске (`run_backup`) и пишет запись в журнал действий. GET для такого
        не годится — его дёргает всё подряд: предзагрузка ссылок браузером,
        закладка, обход админки. Найдено аудитом: обход страниц оставил на
        стенде две лишние копии базы по 8 МБ, ничего для этого не нажимая.
        """
        try:
