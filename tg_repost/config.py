...
class Settings(BaseSettings):
    ...
    # Минимум осмысленного материала (символов, без ссылок и служебных фраз),
    # чтобы вообще запускать рерайт. Если по ссылке НЕ прочитана статья и в
    # оригинале только заголовок — рерайтить нечего, и модель начинает
    # ВЫДУМЫВАТЬ: заголовок CVE «CVE-XXXX ... Information published + URL»
    # превращался в абзацы про «возможную утечку памяти», которых в источнике
    # не было. Для канала по безопасности это публикация выдуманных фактов.
    # Такие посты отсеиваются ДО рерайта (filtered_out), а не раздуваются.
    # 0 — выключить защиту (вернуть старое поведение).
    rewrite_min_source_chars: int = Field(200, alias="REWRITE_MIN_SOURCE_CHARS")
    ...
    # --- F42: реферальная программа (бот Engage) ---
    # Участник приглашает друзей персональной ссылкой и получает очки за тех,
    # кто РЕАЛЬНО остался. Антинакрутка обязательна: без неё механика за день
    # превращается в ферму мультиаккаунтов.
    referrals_enabled: bool = Field(False, alias="REFERRALS_ENABLED")
    # Сколько дней приглашённый должен прожить в группе, чтобы реферал
    # засчитали. Вместе с требованием «написал хотя бы одно сообщение» это и
    # есть вся защита: вступить может кто угодно, а жить и писать — нет.
    referral_min_days: int = Field(3, alias="REFERRAL_MIN_DAYS")

    # --- F44: конкурсы и розыгрыши (бот Engage) ---
    # Розыгрыш воспроизводимый: seed публикуется ДО старта, протокол — после.
    # Без этого аудитория не верит в честность, и конкурс не вовлекает, а
    ...
    # Включён по умолчанию: хэш ловит только буквальный копипаст, а разные
    # источники пишут об одном событии СВОИМИ словами — без эмбеддингов такие
    # повторы проходят насквозь, и это была главная причина дублей в ленте.
    # Цена вопроса невелика (text-embedding-3-small — самая дешёвая модель, и
    # для отсеянных фильтром постов вызов вообще не делается), а при
    # недоступности провайдера приём деградирует мягко: без эмбеддинга пост
    # просто не участвует в семантическом сравнении.
    semantic_dedup_enabled: bool = Field(True, alias="SEMANTIC_DEDUP_ENABLED")
    openai_embedding_model: str = Field(
        "text-embedding-3-small", alias="OPENAI_EMBEDDING_MODEL"
    )
    # 0.88, а не 0.92: на пересказах одного события разными редакциями
    # косинусное сходство обычно 0.85–0.90, и при 0.92 половина реальных
    # повторов проскакивала. Ниже 0.85 начинают слипаться разные новости на
    # общую тему — это заметно хуже, чем пропустить один повтор.
    semantic_similarity_threshold: float = Field(
        0.88, alias="SEMANTIC_SIMILARITY_THRESHOLD"
    )
    dedup_window_days: int = Field(3, alias="DEDUP_WINDOW_DAYS")

    # --- F51: сюжеты (одна новость из нескольких источников) ---
    # Пауза перед рерайтом, чтобы сюжет успел собраться: первый пришедший
    # пост иначе уходит в рерайт сразу, а подтверждения из других источников
    # подтягиваются уже ПОСЛЕ и в фактчек не попадают.
    #
    # По умолчанию 0 (выключено) СОЗНАТЕЛЬНО: это прямая задержка публикации
    # для всех, а от дублей в ленте защищает сам дедуп, а не пауза — она лишь
    # обогащает фактчек. Плюс интервал планировщика уже даёт естественное
    # окно в несколько минут. Включать осмысленно, если источников много и
    # важнее полнота подтверждений, чем скорость выхода.
    cluster_grace_minutes: int = Field(0, alias="CLUSTER_GRACE_MINUTES")
    ...
    # --- RSS как входящий источник ---
    # Лента опрашивается по таймеру, каждая новая запись становится обычным
    # постом со статусом NEW — дальше её обрабатывает тот же пайплайн, что и
    # пост из Telegram-канала (фильтры, стиль источника, переход по ссылке
    # за полным текстом статьи, формат публикации).
    rss_enabled: bool = Field(False, alias="RSS_ENABLED")
    rss_poll_interval_minutes: int = Field(15, alias="RSS_POLL_INTERVAL_MINUTES")
    # Потолок записей за один опрос ОДНОЙ ленты: защита от ленты, которая
    # разом выкатила сотню записей (или сломалась и отдаёт всё подряд).
    rss_max_items_per_poll: int = Field(10, alias="RSS_MAX_ITEMS_PER_POLL")
    # Сколько записей брать при ПЕРВОМ опросе новой ленты. В архиве бывают
    # тысячи записей (у MSRC — больше пяти тысяч), и завести их все постами
    # значит забить очередь модерации и счёт за рерайт. Остальное — история.
    rss_first_poll_items: int = Field(3, alias="RSS_FIRST_POLL_ITEMS")
    # Предохранитель: сколько необработанных постов в очереди достаточно,
    # чтобы приостановить опрос лент. Лент бывает два десятка, каждая отдаёт
    # до `rss_max_items_per_poll` записей за опрос — приток легко обгоняет
    # обработку (пост = вызовы модели + генерация обложек, это секунды), и
    # очередь растёт бесконечно вместе со счётом за API. Найдено на живом
    # стенде: 102 записи заведено за час, обработано за тот же час — ноль.
    # 0 — выключить предохранитель.
    rss_max_queue_backlog: int = Field(200, alias="RSS_MAX_QUEUE_BACKLOG")
    ...
    # долю выручки, не приняв такого решения.
    affiliate_percent: int = Field(0, alias="AFFILIATE_PERCENT")
    ...
    # мешает. Как только включено — рекламный пост БЕЗ erid не публикуется
    # вовсе: опубликовать с половиной маркировки хуже, чем не опубликовать,
    # потому что отозвать ушедший пост нельзя.
    ad_marking_enabled: bool = Field(False, alias="AD_MARKING_ENABLED")
    ...
    # или полностью доверяет редактору, — церемония, которая только
    # замедляет. Включать, когда редактор появился и доверие ещё строится.
    require_owner_approval: bool = Field(False, alias="REQUIRE_OWNER_APPROVAL")
    ...
    # --- Уборка медиа (найдено на стенде 2026-08-18) ---
    # На стенде лежало 2,8 ГБ обложек при базе в 8 МБ, из них 2,3 ГБ — у
    # ОТКЛОНЁННЫХ постов. Ничто их не удаляло, и диск рос до упора.
    # Включено по умолчанию: место кончается тихо, а встаёт после этого всё.
    media_cleanup_enabled: bool = Field(True, alias="MEDIA_CLEANUP_ENABLED")
    # Через сколько дней убирать обложки отработанных постов. У упавших срок
    # ДВОЙНОЙ: их можно повторить из админки, и повтор без картинки — потеря.
    media_retention_days: int = Field(14, alias="MEDIA_RETENTION_DAYS")
    # Сколько дней держать ЗАВЕРШЁННЫЕ задачи очереди (done/failed/canceled).
    # Ждущие и работающие не трогаются никогда: отложенный шаг сценария
    # «напомнить через месяц» выглядит ровно как старая задача.
    # Упавшие уходят по этому же сроку, но их теперь ВИДНО на дашборде —
    # удалять то, чего владелец не имел возможности увидеть, значило бы
    # прятать поломку, а не убирать мусор.
    queue_retention_days: int = Field(30, alias="QUEUE_RETENTION_DAYS")
    # Горизонт журнала действий. Замер на стенде: 47 записей за месяц, места
    # он не занимает — ограничение стоит не ради диска, а потому что система
    # работает без присмотра годами. Срок намеренно большой: журнал
    # подотчётности ценен тем, что помнит давнее. Ноль — не чистить.
    audit_retention_days: int = Field(730, alias="AUDIT_RETENTION_DAYS")

    # --- Автоматические резервные копии (разбор архитектуры 2026-08-18) ---
    # РАНЬШЕ КОПИИ ДЕЛАЛИСЬ ТОЛЬКО КНОПКОЙ, и «хранение 14 последних» из кода
    # не работало в Docker ни дня: каталог не был смонтирован, и каждый деплой
    # стирал их вместе со слоем контейнера. Включено ПО УМОЛЧАНИЮ — копия
    # нужна ровно тогда, когда о ней не подумали заранее.
    backup_enabled: bool = Field(True, alias="BACKUP_ENABLED")
    # Час по UTC. Три ночи: пайплайн в это время спокоен, а копия делается
    # блокирующим чтением файла базы.
