...
ПОЧЕМУ РАСХОЖДЕНИЯ НЕ УСТРАНЕНЫ, А ЗАФИКСИРОВАНЫ. Их 24 (было 26 до
2026-08-18), и ни одно не меняет поведения:
...
* ~~1 × имя индекса~~ — **ПОЧИНЕНО 2026-08-18 миграцией 0052.** Было
  `ix_story_clusters_primary` против `ix_story_clusters_primary_post_id`:
  индекс тот же и на той же колонке, различалось только имя. Расхождение
  безобидное ровно до того дня, когда очередная миграция сошлётся на имя из
  моделей — тогда она упадёт на живой базе и пройдёт в тестах.
...
# Пусто с 2026-08-18: единственное расхождение по индексам (разные имена у
# одного и того же индекса на story_clusters) устранено миграцией 0052.
KNOWN_INDEX_DRIFT: set[str] = set()
...
def test_drift_total_did_not_grow(_diff):
    ...
    assert len(_flatten(_diff)) == 24, (
        f"число расхождений моделей и миграций изменилось: {len(_flatten(_diff))} вместо 24"
    )
